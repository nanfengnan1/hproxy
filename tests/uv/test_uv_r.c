...
void test_msg_cb1(redisAsyncContext *c, void *reply, void *privdata)
{
...
void test_uv_subscribe(redisAsyncContext *context, const char *channel)
{
    ...
    redisAsyncCommand(context, test_msg_cb1, NULL, "%s %s", "subscribe", channel);
}
...
void* pthread_start(void *arg)
{
    ...
    if (redisLibuvAttach(context, uv_loop) < 0) {
